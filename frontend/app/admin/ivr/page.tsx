...
// Import dynamique pour eviter le SSR (react-flow utilise window)
const IVRBuilder = dynamic(() => import('@/components/ivr/IVRBuilder'), { ssr: false })
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
          ...
          ${toast.type === 'ok' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
            ...
            {/* ── IVR BUILDER REACT-FLOW (plein écran) ── */}
            {showBuilder && selectedIVR && (
                <div className="fixed inset-0 z-[60] bg-[#0a0a14] flex flex-col">
                    <div className="flex items-center justify-between px-4 py-2 bg-[#111118] border-b border-[#2e2e44]">
                        <div className="flex items-center gap-3">
                            <button onClick={() => setShowBuilder(false)}
                                className="text-[#55557a] hover:text-[#eeeef8] text-sm">
                                <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6" /></svg>
                            </button>
                            <span className="text-sm font-bold text-[#eeeef8]">Builder visuel : {selectedIVR.name}</span>
                        </div>
                    </div>
                    <div className="flex-1">
                        <IVRBuilder
                            initialNodes={(selectedIVR as any).flow_json?.nodes || []}
                            initialEdges={(selectedIVR as any).flow_json?.edges || []}
                            onSave={async (nodes, edges) => {
                                const flow_json = { nodes, edges }
                                await apiFetch(`/api/v1/admin/ivr/${selectedIVR.id}`, {
                                    method: 'PATCH',
                                    body: JSON.stringify({ flow_json }),
                                })
                                showToast('Flow sauvegarde')
                            }}
                        />
                    </div>
                </div>
