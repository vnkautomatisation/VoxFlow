...
//  v1.2.0 — clean rewrite (Phase D)
//
//  Rôle minimal :
//   - Crée une fenêtre frameless 360x700
...
log('[boot] VoxFlow Dialer v1.2.0 — loading', DIALER_URL)
...
function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        width:  360,
        height: 700,
        x: Math.max(0, screenW - 360 - 16),
        y: Math.max(0, screenH - 700 - 16),
        resizable:       false,
        maximizable:     false,
    ...
    mainWindow.loadURL(DIALER_URL).catch(err => log('[load]', err.message))
...
function createTray() {
    ...
    try {
        ...
        tray.setContextMenu(Menu.buildFromTemplate([
            { label: 'Afficher', click: () => { if (mainWindow) { mainWindow.show(); mainWindow.focus() } } },
            { label: 'Recharger', click: () => mainWindow?.loadURL(DIALER_URL) },
            { type: 'separator' },
